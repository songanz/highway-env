...
class HighwayEnvDis_intrinsic_rew(HighwayEnvDis):
    ...
    def __init__(self, config=None):  # create CVAE model
        ...
        if not config:
            ...
            super(HighwayEnvDis_intrinsic_rew, self).__init__(config)
        state_size = self.observation_space.shape[0]
        try:
            action_size = self.action_space.shape[0]
        except IndexError:  # discrete action, only one
            action_size = 1
    ...
    def step(self, action, fear=False):
        ...
        obs, rew_env, terminal, _ = super(HighwayEnvDis_intrinsic_rew, self).step(action)
    ...
    def intrinsic_rew(self, old_s, s, a, r):
        ...
        """
        a = a/self.action_space.n  # for discrete action
