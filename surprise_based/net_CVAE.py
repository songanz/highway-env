...
def updateBuffers(Buf, s0, act, rew, s1, coli):
    Buf.memory.append([s0, act, rew, s1, coli])
...
class Decoder(nn.Module):
    def __init__(self, hidden_size_IP=100, hidden_size_rest=100, alpha=0.01,
        ...
        self.output = nn.Linear(hidden_size_rest, action_size)
...
class CVAE(nn.Module):
    ...
    def __init__(self, name='network', hidden_size_IP=100, hidden_size_rest=100, alpha=0.01,
        ...
        self.decoder = Decoder(hidden_size_IP, hidden_size_rest, alpha, action_size, latent_size, condi_size)
